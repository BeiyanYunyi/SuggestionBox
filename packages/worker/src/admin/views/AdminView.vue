...
const page = computed(() => Math.max(Number.parseInt(route.params.page as string, 10), 1) - 1)
const { data: tickets, totalPages, canPrev, canNext } = await useSuggestions(page)
...
function goToPage(targetPage: number) {
  router.push({ name: 'admin', params: { page: targetPage + 1 } })
}
  ...
  <main>
    ...
    <div v-if="tickets">
      <Circle v-stylex="styles.forward" color="black">
        <button v-if="canPrev" v-stylex="(styles.btn, styles.btnPrev)" @click.prevent="goToPage(page - 1)">
      ...
      <Circle v-stylex="styles.backward" color="black">
        <button v-if="canNext" v-stylex="(styles.btn, styles.btnNext)" @click.prevent="goToPage(page + 1)">
